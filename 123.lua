...
do
	...
	do
		...
		function Library:New(Properties)
			...
			Logo.Name = "Logo"
			Logo.Image = "http://www.roblox.com/asset/?id=100272022108602"
		...
		function Library:Watermark(Properties)
			local Watermark = {
				Name = (Properties.Name or Properties.name or "lunar.vip | placeholder");
				AnimateText = nil;
			}
			--
			local Outline = Instance.new("Frame")
			...
			task.spawn(function()
				while task.wait() do
					for i = 1, #"lunar.vip" do
						Watermark.AnimateText = string.sub("lunar.vip", 1, i) .. "";
						Title.Text = Watermark.AnimateText .. " " .. Watermark.Name;
						task.wait(0.4);
					end;

					for i = #"lunar.vip" - 1, 1, -1 do
						Watermark.AnimateText = string.sub("lunar.vip", 1, i) .. "";
						Title.Text = Watermark.AnimateText .. " " .. Watermark.Name;
						task.wait(0.4);
					end;
...
--// window locals
local default_color = Color3.fromRGB(156, 0, 255);
...
screen_gui.Name = "https://lunar.vip";
...
screen_gui_2.Name = "https://lunar.vip";
...
do
	...
	do
		...
		instances["local_text"] = utility.instance_new("TextLabel", { 
			Name = "https://lunar.vip",
			Parent = screen_gui_2,
			Text = '<font color="rgb(156, 0, 255)">lunar</font><font color="rgb(255, 255, 255)">.vip</font>',
    ...
    do
        ...
        instances["target_ui"]["logo"] = utility.instance_new("ImageLabel", {
            Name = "Logo";
            Parent = instances["target_ui"]["info"];
            BackgroundColor3 = Color3.fromRGB(156, 0, 255);
            BackgroundTransparency = 1.0;
            BorderColor3 = Color3.fromRGB(0, 0, 0);
            BorderSizePixel = 0;
    ...
    do
		...
		do
			...
			utility.new_connection(run_service.Heartbeat, function()
				...
				if (enabled) then
					...
					local text = custom_text_enabled and '<font color="' .. text_color .. '">' .. features.get_text(custom_text) .. '</font>' or '<font color="' .. text_color .. '">lunar</font><font color="rgb(255, 255, 255)">.vip</font>';
		...
		do
			...
			utility.new_connection(run_service.Heartbeat, function()
				...
				--// lunar classics
				local classics_enabled = flags["anti_aim_lunar_classics_enabled"];
				local classics_keybind = flags["anti_aim_lunar_classics_keybind"];
				local classics_types = flags["anti_aim_lunar_classics"];
				...
				if ((enabled or classics_enabled) and (keybind or classics_keybind) and (not (classics_keybind and classics_enabled and classics_types == "supercoolboi34 Destroyer") or (classics_keybind and classics_enabled and classics_types == "supercoolboi34 Destroyer"))) then
					...
					local types = {
						...
						["supercoolboi34 Destroyer"] = locals.should_lunar_destroy and hrp.CFrame + Vector3.new(0 / 0, 1, math.huge) or hrp.CFrame
					};
			...
			task.spawn(function()
				while task.wait(0.1) do
					locals.should_lunar_destroy = not locals.should_lunar_destroy;
				end;
			end);
...
do
	if (hookmetamethod) then
		...
		local __index; __index = hookmetamethod(game, "__index", LPH_NO_VIRTUALIZE(function(self, property)
			if (not checkcaller() and ((((flags["anti_aim_c_sync_enabled"] or (flags["anti_aim_lunar_classics_enabled"] and (flags["anti_aim_lunar_classics"] == "Destroy Cheaters" or flags["anti_aim_lunar_classics"] == "supercoolboi34 Destroyer"))) and (flags["anti_aim_c_sync_keybind"] or flags["anti_aim_lunar_classics_keybind"]))) and property == "CFrame" and self == local_player.Character.HumanoidRootPart)) then
...
do
	...
	do
		...
		local lunar_classics = ui.tabs["anti_aim"]:Section({Name = "lunar Classics", Side = "Left", Size = 150});
		...
		--// destroy cheaters
		do
			lunar_classics:Toggle({Name = "Enabled", Flag = "anti_aim_lunar_classics_enabled"});
			lunar_classics:Keybind({Flag = "anti_aim_lunar_classics_keybind", Name = "Keybind", Default = Enum.KeyCode.Y, Mode = "Toggle"});
			lunar_classics:List({Name = "Classics", Flag = "anti_aim_lunar_classics", Options = {"Destroy Cheaters", "supercoolboi34 Destroyer"}, Default = "Destroy Cheaters"});
		end;
	end;
	...
	do --// credits to finobe wtv im way too lazy
		...
		local function update_config_list()
		    ...
		    for idx, file in listfiles("lunar/configs") do
		        local file_name = file:gsub("lunar/configs\\", ""):gsub(".cfg", ""):gsub("lunar/configs/", "");
		...
		cfgs:Button({Name = "Create", Callback = function()
		    ...
		    if config_name == "" or isfile("lunar/configs/" .. config_name .. ".cfg") then
		        return;
		    end;
		    writefile("lunar/configs/" .. config_name .. ".cfg", Library:GetConfig());
		    update_config_list();
		end});
		...
		cfgs:Button({Name = "Save", Callback = function()
		    ...
		    if selected_config then
		        writefile("lunar/configs/" .. selected_config .. ".cfg", Library:GetConfig());
		    end;
		end});
		...
		cfgs:Button({Name = "Load", Callback = function()
		    local selected_config = flags.setting_configuration_list;
		    if selected_config then
		        Library:LoadConfig(readfile("lunar/configs/" .. selected_config .. ".cfg"));
		    end;
		end});
		...
		cfgs:Button({Name = "Delete", Callback = function()
		    local selected_config = flags.setting_configuration_list;
		    if selected_config then
		        delfile("lunar/configs/" .. selected_config .. ".cfg");
		    end;
		    update_config_list();
		end});
